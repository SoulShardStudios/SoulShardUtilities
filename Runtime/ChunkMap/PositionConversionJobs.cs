...
namespace SoulShard.Utils
{
    ...
    public partial class ChunkMapInt2D<T>
    {
        ...
        public Vector2Int[] ConvertToInnerChunkPositionsJob(Vector2Int[] positions) =>
            ConvertToSingleChunkPositionsJob<ChunkMapInt2DJobs.InnerChunkPositionConversionJob>(positions);
        public Vector2Int[] ConvertToOuterChunkPositionsJob(Vector2Int[] positions) =>
            ConvertToSingleChunkPositionsJob<ChunkMapInt2DJobs.OuterChunkPositionConversionJob>(positions);
        public Vector2Int[] ConvertToOuterChunkPositionsJobUnique(Vector2Int[] positions) =>
            ConvertToSingleChunkPositionsJob<ChunkMapInt2DJobs.OuterChunkPositionConversionJob>(positions).Distinct().ToArray();
        public ChunkPosition[] ConvertToChunkPositionsJob(Vector2Int[] positions)
        {
            ...
            NativeArray<ChunkPosition> n_inners = ChunkMapInt2DJobs.StandardParallelChunkJob
                <ChunkMapInt2DJobs.ChunkPositionConversionJob, NativeArray<ChunkPosition>>
                (n_positions, chunkSizeV2I, JobUtility.GetBatchAmount(positions.Length, 10, -1), Allocator.TempJob);
        ...
        Vector2Int[] ConvertToSingleChunkPositionsJob<_JobType>(Vector2Int[] positions) where _JobType: struct, IJobParallelFor, IChunkJob<NativeArray<Vector2Int>>
        {
            ...
            NativeArray<Vector2Int> n_positions = new NativeArray<Vector2Int>(positions, Allocator.TempJob);
            NativeArray<Vector2Int> n_inners = ChunkMapInt2DJobs.StandardParallelChunkJob
                <_JobType, NativeArray<Vector2Int>>
                (n_positions, chunkSizeV2I, JobUtility.GetBatchAmount(positions.Length, 10, -1), Allocator.TempJob);
            n_inners.CopyTo(@return);
            n_inners.Dispose();
            n_positions.Dispose();
        ...
        #endregion
        static class ChunkMapInt2DJobs
        {
            #region Jobs
            [BurstCompile]
