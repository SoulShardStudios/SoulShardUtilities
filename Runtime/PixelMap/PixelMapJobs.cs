...
namespace SoulShard.PixelMaps
{
    public partial class PixelMap
    {
        ...
        static class Jobs
        {
            public static _inputType EditJobScedule<_inputType, _jobType>(Vector2Int[] positions, _inputType input, PixelMap map)
                where _jobType : struct, IPixelMapEditJob<_inputType>, IJobParallelFor
            {
                NativeArray<ChunkPosition> n_positions = new NativeArray<ChunkPosition>(map.chunkmap.ConvertToChunkPositionsJob(positions), Allocator.TempJob);
                Vector2Int[] chunks = map.chunkmap.ConvertToOuterChunkPositionsJobUnique(positions);
                NativeArray<JobHandle> jobs = new NativeArray<JobHandle>(chunks.Length, Allocator.TempJob);
                ...
                for (int i = 0; i < chunks.Length; i++)
                {
                    ...
                    // Get the texture.
                    NativeArray<Color32> texture = map.chunkmap.GetChunk(chunks[i]).texture.GetRawTextureData<Color32>();
                    // Run the associated job.
                    _jobType job = new _jobType();
                    job.Init(input, n_positions, texture, map.chunkmap.chunkSizeV2I, chunks[i]);
                    jobs[i] = job.Schedule(n_positions.Length, JobUtility.GetBatchAmount(n_positions.Length, 10, -1));
                }
            ...
            {
                public void Init(inputType @input, NativeArray<ChunkPosition> positions, NativeArray<Color32> texture, Vector2Int chunksize, Vector2Int textureChunkPos);
            }
            #region JobStructs
            [BurstCompile]
            public struct SetPixelsJob : IJobParallelFor, IPixelMapEditJob<NativeArray<Color32>>
            {
                [NativeDisableParallelForRestriction] NativeArray<Color32> texture;
                [ReadOnly] Vector2Int textureChunkPos;
                [ReadOnly] NativeArray<Color32> colors;
                [ReadOnly] NativeArray<ChunkPosition> positions;
                [ReadOnly] Vector2Int chunksize;
                public void Init(NativeArray<Color32> colors, NativeArray<ChunkPosition> positions, NativeArray<Color32> texture, Vector2Int chunksize, Vector2Int textureChunkPos)
                {
                    this.colors = colors;
            ...
            public struct SetPixelsJobSingleColor : IJobParallelFor, IPixelMapEditJob<Color32>
            {
                ...
                [ReadOnly] NativeArray<ChunkPosition> positions;
                [ReadOnly] Vector2Int chunksize;
                public void Init(Color32 color, NativeArray<ChunkPosition> positions, NativeArray<Color32> texture, Vector2Int chunksize, Vector2Int textureChunkPos)
                {
                    this.color = color;
                    this.positions = positions;
                    this.texture = texture;
            ...
            public struct GetPixelsJob : IJobParallelFor, IPixelMapEditJob<NativeArray<Color32>>
            {
                ...
                [ReadOnly] NativeArray<Color32> texture;
                [ReadOnly] Vector2Int textureChunkPos;
                [ReadOnly] NativeArray<ChunkPosition> positions;
                [ReadOnly] Vector2Int chunksize;
                public void Init(NativeArray<Color32> colors, NativeArray<ChunkPosition> positions, NativeArray<Color32> texture, Vector2Int chunksize, Vector2Int textureChunkPos)
